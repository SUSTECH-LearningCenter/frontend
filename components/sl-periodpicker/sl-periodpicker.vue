...
<script>
	...
	export default {
		...
		methods: {
			...
			click(item, name, room) {
				console.log(JSON.stringify(item))
				uni.showToast({
					icon: 'none',
					title: name + '会议室:' + item.start.substring(11, 16) + ' - ' + item.end.substring(11, 16)
				})
			},
			click2(item, name, room) {
				console.log(JSON.stringify(room))
				room.schedule.forEach((item2, index) => {
					if (item.start == item2.start) {
						uni.showModal({
							title: name + '会议室预约情况',
							content: '时间：' + item.start.substring(0, 16) + ' - ' + item.end.substring(11,
								16) + '\n预约人：' + room.schedule[index].user,
							showCancel: false,
							buttonText: '确定',
